...
public class ReabastecerData {
    
    private String mensaje;
    
    public String agregarCompra(Connection con, ReabastecerObj reabastecerObj){
        
        String sqlS = "SELECT id_Compra FROM Compra WHERE id_compra="+(reabastecerObj.getIdCompra())+"";                 
        ...
        try {
            ...
            if (!rs.next()) {
                ...
                String sql = "INSERT INTO Compra(id_compra,id_UsuarioCo,FechaCompra,precioCompra)" + "VALUES(seq_Compra_idCompra.nextval,?,SYSDATE,?)";  
                try {
            
                    pst = con.prepareStatement(sql);
                    pst.setInt(1,3);
                    pst.setFloat(2,reabastecerObj.getPrecioCompra());
    ...
    public String agregarCompraHProd(Connection con, ReabastecerHasProductoObj reabastecerHProdObj){
        
        PreparedStatement pst = null;
        
        
            String sql = "INSERT INTO Compra_has_Producto(id_compra,ID_Producto,cantidad)" + " VALUES((SELECT id_Compra FROM (select * from Compra ORDER BY id_Compra desc ) WHERE rownum = 1),?,"+Float.valueOf(reabastecerHProdObj.getCantidad())+")";  
             try {
            
                pst = con.prepareStatement(sql);
                pst.setInt(1,reabastecerHProdObj.getIdProducto());
    ...
    public String actualizarStock(Connection con, ReabastecerHasProductoObj reabastecerHProdObj, Float antExis){
        
        PreparedStatement pst = null;
        
        
            String sql = "UPDATE Producto SET Existencia="+(reabastecerHProdObj.getCantidad()+antExis)+" WHERE id_Producto = ?;";  
            try {
